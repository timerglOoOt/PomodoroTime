...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        appRouter.currentTabBarController = window?.rootViewController as? TabBarController
    }
    ...
    func openScreen(with queryItems: [URLQueryItem]) {
        ...
        case "new":
            let taskName = queryItems.first(where: { $0.name == "name" })?.value
            appRouter.routeTo(.new(name: taskName))
        default:
            break
        }
    }
